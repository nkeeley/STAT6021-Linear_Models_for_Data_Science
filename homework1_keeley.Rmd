...
The correlation between eruption time and waiting time is 0.901. This correlation, in conjunction with visual cues from the scatter plot produced earlier, suggests a strong positive correlation between eruption duration and wait time.
...
NOTE: In this document, the " ' " symbol is sometimes used to symbolize a "hat," such as in the estimate of the intercept B0'.\newline
...
Bˆ1 is the estimated coefficient of the predictor variable for this linear regression model, and Bˆ0 is the estimated intercept of the linear regression model. In this context, Bˆ1 represents the change in the estimated waiting time for the next eruption for each additional minute of the last eruption duration. Bˆ0 represents the estimated mean of the distribution of the response variable Yi (wait time) when the predictor variable Xi is equal to 0. However, this value has limited use in this context, since the range of observed X values (eruption duration) does not extend to 0.
...
The 95% confidence interval for the slope: 10.10983 <= B1 <= 11.34937
...
95% prediction interval for x=3.5: 59.36332 <= Y <= 82.69296
...
95% confidence interval for mean response at x=3.5: 70.32211 <= E(avgY|X=3.5) <= 71.73417
